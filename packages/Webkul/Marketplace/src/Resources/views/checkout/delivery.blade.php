    ...
    <div class="mx-auto max-w-[1200px] px-4 py-8">
        ...
        <div class="grid min-w-0 gap-8 lg:grid-cols-[1fr_360px]">
            <div class="min-w-0">
                ...
                <p class="mb-6 text-sm text-slate-500">Real quotes for your route, priced by distance and vehicle type.</p>

                <form id="delivery-form" method="POST" action="{{ route('marketplace.checkout.delivery.store') }}">
                    @csrf

                    @foreach ($sellersWithQuotes as $row)
                        <div class="mb-6 rounded-xl border border-slate-200 p-4 sm:p-5">
                            <p class="mb-1 font-semibold text-slate-800">{{ $row['seller']->shop_name }}</p>
                            <p class="mb-4 text-xs text-slate-500">{{ $row['product_names']->implode(', ') }}</p>
                            ...
                            @else
                                <div class="grid gap-3">
                                    @foreach ($row['quotes'] as $index => $entry)
            ...
            <aside class="h-fit min-w-0 rounded-xl border border-slate-200 p-5 lg:sticky lg:top-24">
                ...
                <div class="flex items-center justify-between border-t border-slate-200 pt-3 text-sm">
                    <span class="font-semibold text-slate-800">Total delivery fee</span>
                    <span class="text-lg font-bold text-brandGreen" id="delivery-total">
                        {{ core()->formatPrice(collect($sellersWithQuotes)->sum(fn ($row) => $row['quotes']->first()['quote']->feeMinor ?? 0) / 100) }}
                    </span>
                </div>
                ...
                <button
                    type="submit"
                    form="delivery-form"
                    {{ collect($sellersWithQuotes)->every(fn ($row) => $row['quotes']->isNotEmpty()) ? '' : 'disabled' }}
        ...
        (function () {
            ...
            function recalculate() {
                ...
                document.querySelectorAll('.delivery-radio:checked').forEach(radio => {
                    sellerFees[radio.dataset.sellerId] = parseFloat(radio.dataset.feeMinor) / 100;
                });

                const total = Object.values(sellerFees).reduce((sum, fee) => sum + fee, 0);
                document.getElementById('delivery-total').textContent = formatPrice(total);
